...
void Road::Draw()
{
	...
	glPushMatrix();
	glTranslatef(bLength, 0.0f, 1.5 * bWidth);
	...
	glPushMatrix();
	glTranslatef(bLength, 0.0f, 2.5 * bLength - zebraLength / 2);
	for (int i = -1; i < 2; i++)
	{
		glPushMatrix();
	...
	glPushMatrix();
	glTranslatef(-2 * bLength, height, 1.5 * bWidth + sidewalkWidth / 2);
	glColor3f(0.5f, 0.5f, 0.5f);
	DrawCuboid(4 * bLength, sidewalkWidth + 2 * bWidth, sidewalkHeight);
	glPopMatrix();

	glPushMatrix();
	glTranslatef(bLength, height, 2.5 * bWidth + sidewalkWidth / 2);
	glColor3f(0.5f, 0.5f, 0.5f);
	DrawCuboid(2 * bLength, sidewalkWidth, sidewalkHeight);
	glPopMatrix();

	glPushMatrix();
	glTranslatef(3 * bLength, height, 1.5 * bWidth + sidewalkWidth / 2);
	glColor3f(0.5f, 0.5f, 0.5f);
	...
	// Draw the graassland
	glPushMatrix();
	glTranslatef(-3 * bLength - sidewalkWidth / 2, height, 1.5 * bWidth + sidewalkWidth);
